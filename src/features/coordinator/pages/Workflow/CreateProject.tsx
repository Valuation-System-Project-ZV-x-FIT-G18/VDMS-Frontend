...
const CreateProject: React.FC = () => {
  ...
  useEffect(() => {
    if (!mapRef.current) {
      const map = L.map('map').setView([selectedLocation.lat, selectedLocation.lng], 11);
      ...
      setMapCoords(`Selected: ${selectedLocation.lat.toFixed(4)}, ${selectedLocation.lng.toFixed(4)}`);
    }
  ...
  const updateLocation = (lat: number, lng: number) => {
    setSelectedLocation({
      lat,
      lng,
      address: generateAddressFromCoords(lat, lng, street, city, district, province)
    });
  ...
  useEffect(() => {
    setSelectedLocation(prev => ({
      ...prev,
      address: generateAddressFromCoords(prev.lat, prev.lng, street, city, district, province)
    }));
  ...
  const handleProvinceChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (!value) {
      ...
      setProvinceError('');
      if (provinceDistrictMap[value] && !provinceDistrictMap[value].includes(district)) {
        setDistrict('');
        ...
        <div className="form-grid">
          ...
          <div className="form-card">
            ...
            <div className="district-hint" id="provinceHint">
              {province ? `Showing districts in ${province} Province` : 'Select a province to filter districts'}
            </div>

            <div className="form-group">
              <div className="map-label">Choose On Map</div>
              <div className="map-container" id="mapContainer">
                <div id="map" style={{ height: '200px', width: '100%', zIndex: 1 }}></div>
              </div>
              ...
              <div className="location-info">
                ...
                <div className="location-note">Click on map or drag marker to select different location</div>
              </div>
            </div>
          </div>
          ...
          <div className="form-card">
            <h2 className="form-card__title with-icon">
              <svg viewBox="0 0 24 24" width="18" height="18">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" stroke="currentColor" fill="none" />
                <path d="M14 2v6h6" strokeWidth="2" stroke="currentColor" fill="none" />
                <path d="M12 18v-6" strokeWidth="2" strokeLinecap="round" stroke="currentColor" fill="none" />
                <path d="M9 15h6" strokeWidth="2" strokeLinecap="round" stroke="currentColor" fill="none" />
              </svg>
              Additional Details
            </h2>
            <div className="form-group">
