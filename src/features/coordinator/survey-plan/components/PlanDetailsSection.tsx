...
const PlanDetailsSection = ({ form, onChange, errors = {} }: Props) => (
  <div className="plan-details-section">
    <SectionHeader icon="📋" title="Plan details" />
    <FormField label="Survey plan number" name="planNumber" value={form.planNumber}
      onChange={onChange} placeholder="e.g. SP/2024/001" error={errors.planNumber} required />
    <div className="plan-row">
      <FormField label="Surveyor name" name="surveyorName" value={form.surveyorName}
        onChange={onChange} placeholder="e.g. K. Perera" error={errors.surveyorName} required half />
      <FormField label="Lot number" name="lotNumber" value={form.lotNumber}
        onChange={onChange} placeholder="e.g. Lot 12A" error={errors.lotNumber} required half />
    </div>
  </div>
);
